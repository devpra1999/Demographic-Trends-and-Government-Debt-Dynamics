...
sfa_plot <- ggplot(df_plot %>% filter(Year >= 1960), aes(x = Year, y = sfa, color = Country)) +
...
for (ctry in unique(df_plot$Country)){
  print(paste0(ctry," ",round(mean(df_plot %>% filter(Country == ctry) %>% filter(Year >= 2000) %>% pull(sfa),na.rm = T),4)))
}
...
pi_plot <- ggplot(df_plot %>% filter(Year > 1960), aes(x = Year, y = pi, color = Country)) +
  geom_line(linewidth = 1) +
  labs(
    y = "Inflation",
    x = "Year") +
  ...
  theme(text=element_text(size=15))
pi_plot
ggsave("plots/desc_inflation_big5.pdf",pi_plot)
