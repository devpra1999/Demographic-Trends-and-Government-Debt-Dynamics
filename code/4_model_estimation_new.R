...
df_output <- df %>%
  select(Country,Year,y,ypot,dy,dypot,n,dn, pi, D,D_Y,D_R,ygap,ygap_lag) %>%
  mutate(g = dy + pi) %>%
...
df_debt <- df %>%
  select(Country,Year,y,dy,ypot,dypot,ygap,pi,MY,D,D_Y,D_R,IR,PTR,Total_Debt,Expenditure_Interest,GDP,Surplus,Primary_Surplus) %>%
  mutate(PTR = PTR/100,
         #IR_r = IR - PTR,
         pi_target = 0.02,
         b = Total_Debt/GDP,
...
# #Restricted Model
# regressors <- c("(Intercept)","ygap","b_lag","D_R")
# vars_to_restrict <- c("b_lag")
# restr <- build_restriction_matrix(vars_to_restrict, names(country_map), regressors)
# mod_s_restricted <- systemfit(eqns, data = df_debt %>% filter(Year>1990), method = "OLS",
#                                   restrict.matrix = restr$R, restrict.rhs = restr$q)
# summary(mod_s_restricted)
